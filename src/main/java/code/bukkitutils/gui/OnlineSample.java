...
public class OnlineSample {
    ...
    public void getPage(UUID sender, Integer page) {

        manager.getPlugin().getLogger().info(String.valueOf(page));
        ...
        manager.getPlugin().getLogger().info(titlename);
        ...
        for (UUID uuid : listPlayers) {
            ...
            inventory.addHead(player.getName(), name, lore);
            number++;
        }


        int pagegui = inventory.getInv().getSize();
        ...
        inventory.setItem(pagegui - 9, command.getString("commands.msg-online.previous-page.id_name"), previousName, previousLore);
